...
def import_module(module_type: str):
    """
    Dynamically import the specified module
    
    Args:
        module_type: Type of module to import ('original' or 'optimized')
        
    Returns:
        Tuple of (success, error_message, module_function)
    """
    try:
        ...
        # Determine which module file to import
        if module_type == "original":
            module_file = os.path.join(utils_path, "enhanced_research.py")
            module_name = "enhanced_research"
        else:  # optimized
            if os.path.exists(os.path.join(utils_path, "enhanced_research_optimized.py")):
                module_file = os.path.join(utils_path, "enhanced_research_optimized.py")
                module_name = "enhanced_research_optimized"
            else:
                module_file = os.path.join(utils_path, "new_enhanced_research.py")
                module_name = "new_enhanced_research"
        
        # Check if the module file exists
        if not os.path.exists(module_file):
            return False, f"{module_type.capitalize()} module not found at {module_file}", None
        
        # Import the module dynamically
        import importlib.util
        spec = importlib.util.spec_from_file_location(module_name, module_file)
        if spec is None:
            return False, f"Failed to load module spec from {module_file}", None
            
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        
        # Get the generate_enhanced_research_content function
        if not hasattr(module, "generate_enhanced_research_content"):
            return False, f"Function generate_enhanced_research_content not found in {module_file}", None
            
        func = getattr(module, "generate_enhanced_research_content")
        return True, None, func
        
    except Exception as e:
        error_msg = f"Error importing module {module_type}: {str(e)}\n{traceback.format_exc()}"
...
def run_research_query(
    ...
    try:
        # Import the module
        success, error_msg, generate_func = import_module(module_type)
