#!/usr/bin/env python3
"""
FINAL WORKING Pinecone Document Ingestion
Uses pinecone-client v4.1.2 correctly
"""
...
# Page config
st.set_page_config(
    page_title="Final Working Pinecone",
    page_icon="✅",
    layout="wide"
)

st.title("✅ Final Working Pinecone Document Ingestion")
st.markdown("**Using pinecone-client v4.1.2 - This WILL work!**")
...
# Import Pinecone v4.1.2
try:
    from pinecone import Pinecone, ServerlessSpec
    st.success("✅ pinecone-client v4.1.2 imported successfully")
except ImportError as e:
    st.error(f"❌ Import failed: {e}")
    st.error("Run: py -m pip install pinecone-client==4.1.2")
    st.stop()

# Connection test
with st.expander("🔍 Connection Test", expanded=True):
    if st.button("Test Final Connection"):
        try:
...
if uploaded_file:
    ...
    # Ingestion button
    if st.button("🚀 START FINAL INGESTION", type="primary"):
        ...
        try:
            ...
            if index_name not in existing_names:
                ...
                import time
                status.text("⏳ Waiting for index to be ready...")
            ...
            status.text("✅ FINAL INGESTION COMPLETED!")
            
            # Success celebration
            st.success("🎉 **FINAL INGESTION SUCCESSFUL!**")
            st.balloons()
            ...
            st.error(f"❌ Final ingestion failed: {e}")
            import traceback
            st.code(traceback.format_exc())

# Footer
st.markdown("---")
st.markdown("**✅ This final version uses pinecone-client v4.1.2 correctly!**")
st.markdown("**No more package conflicts - this WILL work!**")
